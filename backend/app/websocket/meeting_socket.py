...
async def _process_audio_chunk(
    ...
    try:
        ...
        logger.info(
            "Incoming audio_stream -> meeting_id=%s user_id=%s "
            "received_source_language=%s",
            meeting_id,
            user_id,
            received_source_language,
        )
        configured_source_language = received_source_language or translation_service.get_language_code(
        ...
        logger.info(
            "Language Debug -> stored_preferred=%s, stored_source=%s, "
            "received_source=%s, configured=%s",
            stored_preferred_language,
            stored_source_language,
            received_source_language,
        ...
        if not configured_source_language:
            logger.error(
                ...
                "meeting_id=%s user_id=%s chunk_id=%s",
                meeting_id,
                user_id,
                chunk_id,
            )
        ...
        # TEMP TRACE LOGGING (realtime pipeline investigation; safe to
        # remove once subtitles are confirmed working end-to-end again).
        # This is stage 5 of the pipeline trace:
        #   Audio received -> Deepgram request -> Deepgram response ->
        #   transcribe() return value -> [meeting_socket received transcript]
        #   -> WebSocket payload sent
        print(
            f"[TRACE-5-MEETING-SOCKET-RECEIVED] chunk_id={chunk_id} "
            f"transcript_result={transcript_result!r}"
        )
        ...
        # TEMP TRACE LOGGING (realtime pipeline investigation; safe to
        # remove once subtitles are confirmed working end-to-end again).
        # This is stage 6 of the pipeline trace (final stage):
        #   ... -> meeting_socket received transcript ->
        #   [WebSocket payload sent]
        print(
            f"[TRACE-6-WEBSOCKET-PAYLOAD-SENT] chunk_id={chunk_id} "
            f"recipients={transcript_recipients} payload={transcript_payload!r}"
        )
...
async def _postprocess_transcript(meeting_id: str, job: dict):
    ...
    async def translate_and_deliver(
        ...
        if not target_code:
            ...
            async with translation_model_lock:
                logger.info(
                    "Translation Debug -> source_lang=%s, target_lang=%s",
                    job["source_language"],
                    language,
                )
                result = await asyncio.to_thread(
    ...
    try:
        while True:
            ...
            # ==========================================
                ...
                if not corrected_source_language:
                    logger.error(
                        "Correction translation rejected because source language was "
                        "missing: meeting_id=%s chunk_id=%s",
                        meeting_id,
                        chunk_id,
                    )
                    continue
                ...
                correction_event = {
                    ...
                    "language": corrected_source_language,
                ...
                correction_job = {
                    ...
                    "source_language": corrected_source_language,
