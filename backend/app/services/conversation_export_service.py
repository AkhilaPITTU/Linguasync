...
async def export_conversation_pdf(meeting_id: str, requester_id: str):
    ...
    print(f"[EXPORT] preferred_language: {preferred_language}")
    ...
    translation_count = await translations_collection.count_documents({"meeting_id": meeting_id})
    print(f"[EXPORT] transcripts found: {transcript_count}")
    print(f"[EXPORT] translations found: {translation_count}")
    ...
    async for record in cursor:
        ...
        if not text:
            continue
        source_language = _clean_text(record.get("source_language") or record.get("language"))
        if not source_language:
            raise ConversationExportError(422, "A transcript is missing its detected source language and cannot be exported safely.")
        ...
        if not translated_text:
            ...
            if not translation.get("success") or not translated_text:
                raise ConversationExportError(422, "A conversation entry could not be translated for your preferred language. Please try again.")
    ...
    print(f"[EXPORT] conversation entries: {len(conversation)}")
    print("[EXPORT] generating PDF")
    pdf_bytes = _build_pdf(meeting_id, requester_name, preferred_language, conversation)
    print(f"[EXPORT] PDF generated successfully")
    print(f"[EXPORT] PDF size: {len(pdf_bytes)} bytes")
